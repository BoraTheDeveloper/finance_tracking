...
export function shiftIsoMonth(month: string, delta: number) {
  ...
  return isoMonthFromDay(
    isoDayFromDate(new Date(year, monthIndex - 1 + delta, 1)),
  );
}
...
export function daysUntilDue(dueDay: number) {
  ...
  if (due.getTime() < new Date(year, month, today.getDate()).getTime())
    due = new Date(year, month + 1, dueDay);
  return Math.round(
    (due.getTime() - new Date(year, month, today.getDate()).getTime()) /
      86400000,
  );
}
