...
export function summarizeModelForDay(
  model: AppModel,
  day: string,
  rolloverUsd: number,
) {
  const cycleTransactions = filterExpensesByBudgetCycle(
    model.expenses,
    day,
    model.budgetCycleStartDay,
  );
  const monthExpenses = cycleTransactions.filter(isExpenseTransaction);
  const monthIncomeUsd = cycleTransactions
  ...
  return summarizeBudget({
    ...
    categorySpend: model.categories.map((category) => ({
      key: category.key,
      spent: money(
        Math.round((monthSpendByCategory.get(category.key) ?? 0) * 100),
        "USD" as const,
      ),
      budget: money(Math.round(category.budgetUsd * 100), "USD" as const),
    })),
    todayExpenses: filterExpensesByDay(model.expenses, day)
    ...
    daysLeftIncludingToday: daysRemainingInBudgetCycle(
      day,
      model.budgetCycleStartDay,
    ),
...
export function rolloverUsdAfterElapsedDays(
  model: AppModel,
  elapsedDays: readonly string[],
) {
