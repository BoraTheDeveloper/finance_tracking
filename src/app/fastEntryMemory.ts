...
const AMOUNT_AND_CURRENCY_WORDS = /(?:[$៛]|\busd\b|\bdollars?\b|\bbucks?\b|\bkhr\b|\briels?\b|\d[\d,]*(?:\.\d+)?)/gi;

export type QuickAmountChip = Readonly<{ label: string; text: string; cur: Currency }>;
...
export function buildFastEntryMemory(expenses: readonly Expense[], categories: AppModel['categories']): FastEntryMemory {
  const memory: FastEntryMemory = {};
  let memorySize = 0;

  for (const expense of expenses) {
    if (!isExpenseTransaction(expense) || !categories.some((category) => category.key === expense.cat)) continue;
...
export function rememberedFastEntryCategory(name: string, memory: FastEntryMemory, categories: AppModel['categories']) {
  const key = normalizeFastEntryName(name);
  const remembered = key ? memory[key] : undefined;
  if (!remembered || !categories.some((category) => category.key === remembered.cat)) return null;
