...
export function shouldRefreshExchangeRate(lastFetchedDay: string | null | undefined, today = isoDayFromDate(new Date())) {
...
export function parseUsdToKhrRate(payload: unknown) {
  ...
  return typeof khrPerUsd === 'number' && Number.isFinite(khrPerUsd) && khrPerUsd > 0 ? khrPerUsd : null;
}

export async function fetchUsdToKhrRate(fetcher: ExchangeRateFetcher = fetch): Promise<UsdToKhrRateResult | null> {
