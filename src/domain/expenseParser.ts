...
export function parseExpenseText(input: string): ParsedExpense {
  ...
  const amountText = text.includes(",")
    ? text.split(",").slice(1).join(",").toLowerCase()
    : lower;
  const match =
    amountText.match(/(\d[\d,]*\.?\d*|\d*\.\d+)/) ??
    lower.match(/(\d[\d,]*\.?\d*|\d*\.\d+)/);
  ...
  return {
    amount:
      amountNumber == null ? null : parseMoney(String(amountNumber), currency),
