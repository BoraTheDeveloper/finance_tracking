...
function categoryLabel(categoryKey: string, categories: readonly Pick<Category, 'key' | 'label'>[] | undefined) {
  if (categoryKey === 'income') return 'Income';
  return categories?.find((category) => category.key === categoryKey)?.label ?? categoryKey;
}

export function transactionSearchHaystack(transaction: Expense, categories: readonly Pick<Category, 'key' | 'label'>[] = []) {
...
export function filterTransactions(transactions: readonly Expense[], filters: TransactionFilterInput) {
  ...
  return transactions.filter((transaction) => {
    ...
    return !text || transactionSearchHaystack(transaction, filters.categories).includes(text);
  });
}
