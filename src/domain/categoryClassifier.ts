...
import {
  WEIGHTS,
  type CategoryClassifierWeights,
} from "./categoryClassifierWeights";

export type CategoryKindHint =
  "" | "purchase" | "transfer_in" | "transfer_out" | "other";
...
export type CategoryClassifierResult = Readonly<{
  ...
  alternatives: ReadonlyArray<
    Readonly<{ categoryKey: string; confidence: number }>
  >;
  cleanLabel: string;
}>;
...
function amountBucket(
  amount: number,
  buckets: CategoryClassifierWeights["amountBuckets"],
) {
...
function buildFeatureWeights(
  ...
  weights: CategoryClassifierWeights,
) {
...
function applyRules(
  ...
  if (
    ...
    amount > 0 &&
    amount < 3 &&
    Math.max(...Object.values(next)) < 0.6
  ) {
...
function softmax(scores: Record<string, number>, temperature: number) {
  ...
  const scaled = classes.map(
    (key) => (scores[key] - max) * Math.max(temperature, 0.1),
  );
  ...
  return Object.fromEntries(
    classes.map((key, index) => [key, exps[index] / sum]),
  );
}
...
export function predictCategory(
  ...
  for (const classKey of weights.classes) {
    ...
    const keywordBonus =
      ...
      keywordMatches(input.cleanLabel, weights.classKeywords[classKey] ?? []);
  ...
  const ranked = weights.classes
    .map((categoryKey) => ({
      categoryKey,
      confidence: probs[categoryKey] ?? 0,
    }))
  ...
  const keywordOnly =
    keywordMatches(
      input.cleanLabel,
      weights.classKeywords[top.categoryKey] ?? [],
    ) > 0;
