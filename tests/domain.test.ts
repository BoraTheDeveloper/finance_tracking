...
describe("money", () => {
  it("stores USD cents and KHR whole riel", () => {
    expect(parseMoney("10.25", "USD")).toEqual({
      amountMinor: 1025,
      currency: "USD",
    });
    expect(parseMoney("1000", "KHR")).toEqual({
      amountMinor: 1000,
      currency: "KHR",
    });
  });
  ...
  it("converts KHR/USD deterministically at 4100", () => {
    expect(
      convertMoney(money(4000, "KHR"), "USD", { khrPerUsd: 4100 }),
    ).toEqual(money(98, "USD"));
    expect(
      convertMoney(money(1000, "USD"), "KHR", { khrPerUsd: 4100 }),
    ).toEqual(money(41000, "KHR"));
  });

  it("rejects non-positive or non-finite exchange rates before conversion", () => {
    expect(() =>
      convertMoney(money(1000, "KHR"), "USD", { khrPerUsd: 0 }),
    ).toThrow("Exchange rate must be positive");
    expect(() =>
      convertMoney(money(1000, "KHR"), "USD", { khrPerUsd: -4100 }),
    ).toThrow("Exchange rate must be positive");
    expect(() =>
      convertMoney(money(1000, "KHR"), "USD", { khrPerUsd: Number.NaN }),
    ).toThrow("Exchange rate must be positive");
  });
...
describe("fast entry helpers", () => {
  ...
  it("remembers the latest saved expense category by normalized merchant name", () => {
    ...
    expect(
      rememberedFastEntryCategory("lucky burger", memory, categories),
    ).toBe("food");
  });
  ...
  it("appends and replaces quick chip amounts as parseable transaction text", () => {
    expect(
      parseExpenseText(applyQuickAmountChip("coffee", "5$")).amount,
    ).toEqual(money(500, "USD"));
    expect(
      parseExpenseText(applyQuickAmountChip("coffee, 2$", "5000 riels")).amount,
    ).toEqual(money(5000, "KHR"));
  });
});
...
describe("insight heatmap helpers", () => {
  it("classifies no spend, low spend, enough budget, and overspend at thresholds", () => {
    ...
    expect(cells.map((cell) => cell.spentUsdCents)).toEqual([
      0, 500, 850, 1000, 1300, 1301,
    ]);
  });
  ...
  it("uses only the latest 35 days and labels today when the visible history reaches today", () => {
    ...
    expect(longCells[0]).toMatchObject({
      index: 0,
      spentUsdCents: 600,
      label: "Day 1",
    });
    ...
    const fullVisibleCells = buildHeatmap(
      Array.from({ length: 35 }, (_item, index) => index + 1),
      100,
    );
    expect(fullVisibleCells[34]).toMatchObject({
      index: 34,
      spentUsdCents: 3500,
      label: "Today",
    });
