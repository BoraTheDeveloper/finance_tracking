...
describe("app JSON backups", () => {
  ...
  it("rejects non-JSON input", () => {
    expect(() => parsePersistedBackup("not json")).toThrow(
      "Backup file is not valid JSON",
    );
  });

  it("rejects JSON without a numeric version and object state", () => {
    expect(() => parsePersistedBackup('{"version":"1","state":{}}')).toThrow(
      "persisted Luy Khnom JSON envelope",
    );
    expect(() => parsePersistedBackup('{"version":1,"state":[]}')).toThrow(
      "persisted Luy Khnom JSON envelope",
    );
  ...
  it("derives restore preview counts and backup date from a parsed envelope", () => {
    const state = {
      ...
      expenses: [
        { id: "expense-1", date: "2026-07-06" },
        { id: "expense-2", date: "2026-07-07" },
      ],
      goals: [{ id: "goal-1" }],
      ious: [],
      recurringPayments: [{ id: "recurring-1" }],
    };
    const backup = parsePersistedBackup<typeof state>(
      stringifyPersistedBackup(state),
    );
  ...
  it("falls back to latest expense date and zeroes missing preview collections", () => {
    const backup = parsePersistedBackup<{ expenses: { date?: unknown }[] }>(
      stringifyPersistedBackup({
        expenses: [
          { date: "2026-07-05" },
          { date: "not-a-day" },
  ...
  it("refuses to preview envelopes whose state is not an object", () => {
    expect(
      deriveRestorePreview({ version: PERSISTED_APP_STATE_VERSION, state: [] }),
    ).toBeNull();
  });

  it("creates a share-safe JSON file name", () => {
    expect(backupFileName(new Date("2026-07-06T01:02:03.004Z"))).toBe(
      "luy-khnom-backup-2026-07-06T01-02-03-004Z.json",
    );
  });
});
