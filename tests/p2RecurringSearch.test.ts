...
describe("local transaction filters", () => {
  it("matches transaction name, note, category label, and type text locally", () => {
    expect(
      filterTransactions(transactions, { text: "net", categories }).map(
        (transaction) => transaction.id,
      ),
    ).toEqual(["tx-1"]);
    expect(
      filterTransactions(transactions, { text: "payday", categories }).map(
        (transaction) => transaction.id,
      ),
    ).toEqual(["tx-2"]);
    expect(
      filterTransactions(transactions, { text: "food", categories }).map(
        (transaction) => transaction.id,
      ),
    ).toEqual(["tx-3"]);
    expect(
      filterTransactions(transactions, { text: "income", categories }).map(
        (transaction) => transaction.id,
      ),
    ).toEqual(["tx-2"]);
    expect(
      filterTransactions(transactions, { text: "outcome", categories }).map(
        (transaction) => transaction.id,
      ),
    ).toEqual(["tx-1", "tx-3"]);
  });
...
describe("recurring payment helpers", () => {
  ...
  it("returns unpaid recurring payments in due-day order", () => {
    ...
    expect(
      upcomingRecurringPayments(payments, {
        [recurringPaidBillKey("gym")]: true,
