...
describe("onboarding step migration", () => {
  it("starts fresh installs at welcome without remapping old setup steps", () => {
    ...
    expect(normalizeLoadedModel(undefined, TODAY).onbStep).toBe(
      WELCOME_ONBOARDING_STEP,
    );
    ...
    for (const step of [
      ...
      expect(
        normalizeLoadedModel(
          { onboarded: false, screen: "onboarding", onbStep: step },
          TODAY,
        ).onbStep,
      ).toBe(step);
    }
  ...
  it("keeps restored onboarded users on their persisted screen instead of forcing welcome", () => {
    const restored = normalizeLoadedModel(
      { onboarded: true, screen: "home", onbStep: WELCOME_ONBOARDING_STEP },
      TODAY,
    );
  ...
  it("normalizes invalid persisted onboarding steps to a reachable setup boundary", () => {
    ...
    expect(normalizeLoadedModel({ onbStep: -9 } as never, TODAY).onbStep).toBe(
      WELCOME_ONBOARDING_STEP,
    );
    expect(normalizeLoadedModel({ onbStep: 99 } as never, TODAY).onbStep).toBe(
      LAST_SETUP_ONBOARDING_STEP,
    );
  });
});
...
describe("budget cycle migration", () => {
  it("defaults missing or invalid cycle start days to the 1st", () => {
    ...
    expect(
      normalizeLoadedModel({ budgetCycleStartDay: 0 }, TODAY)
        .budgetCycleStartDay,
    ).toBe(1);
    expect(
      normalizeLoadedModel({ budgetCycleStartDay: 32 }, TODAY)
        .budgetCycleStartDay,
    ).toBe(1);
    expect(
      normalizeLoadedModel({ budgetCycleStartDay: 5 }, TODAY)
        .budgetCycleStartDay,
    ).toBe(5);
  });
  ...
  it("normalizes the active cycle key from the budget cycle start day", () => {
    expect(
      normalizeLoadedModel(
        { budgetCycleStartDay: 5, lastActiveMonth: null },
        "2026-08-04",
      ).lastActiveMonth,
    ).toBe("2026-07");
  });
...
describe("default category migration", () => {
  it("seeds legacy empty category state without creating personal records", () => {
    const normalized = normalizeLoadedModel(
      { defaultCategoriesSeeded: false, categories: [] },
      TODAY,
    );

    expect(normalized.defaultCategoriesSeeded).toBe(true);
    expect(normalized.categories.map((category) => category.key)).toEqual(
      INITIAL_MODEL.categories.map((category) => category.key),
    );
  ...
  it("preserves a user-cleared category list once defaults were already seeded", () => {
    const normalized = normalizeLoadedModel(
      { defaultCategoriesSeeded: true, categories: [] },
      TODAY,
    );
  ...
  it("preserves custom categories during normalization", () => {
    ...
    const normalized = normalizeLoadedModel(
      { defaultCategoriesSeeded: false, categories: customCategories },
      TODAY,
    );
