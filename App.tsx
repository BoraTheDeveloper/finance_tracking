...
import { AnimatedCue, AnimatedSplitBar, EmptyState, Pill, Progress, ReceiptToRingCue, RingBudget, Row, SectionHeader, SelectableChip, SetupDot, Stat, Upcoming } from './src/ui/components';
...
import { hapticError, hapticSelect, hapticSuccess, hapticWarning } from './src/ui/haptics';
...
function AppContent() {
  ...
  const monthTransactions = filterExpensesByBudgetCycle(model.expenses, today, model.budgetCycleStartDay);
  ...
  const chartHistoryValues = [...model.history, budget.spentTodayUsd.amountMinor / 100].slice(-35);
  const chartHistoryItems = chartHistoryValues.map((value, index) => ({
    date: addIsoDays(today, index - (chartHistoryValues.length - 1)),
    value,
  }));
  const cycleHistoryItems = chartHistoryItems.filter((item) => item.date >= budgetCycle.start && item.date <= today);
  ...
  const selectedDayItem = cycleHistoryItems[selectedDayIndex] ?? { date: today, value: budget.spentTodayUsd.amountMinor / 100 };
  const selectedDayDate = selectedDayItem.date;
  const selectedDayTransactions = filterExpensesByDay(model.expenses, selectedDayDate);
  const transactionMonthFilter = drafts.transactionMonth || 'current-cycle';
  const transactionHistoryBase = transactionMonthFilter === 'current-cycle'
    ? monthTransactions
  ...
  const transactionHistoryMonths = ['current-cycle', thisMonth, shiftIsoMonth(thisMonth, -1), shiftIsoMonth(thisMonth, -2)];
  const transactionHistoryFilterActive = Boolean(drafts.transactionSearch.trim() || drafts.transactionTypeFilter !== 'all' || drafts.transactionCategoryFilter || transactionMonthFilter !== 'current-cycle');
  const upcomingRecurring = upcomingRecurringPayments(model.recurringPayments, model.paidBills, model.billReminders);
  ...
  function addExpense() {
    ...
    showToast(`Added ${isIncome ? 'income' : expense.name}`, 'Undo', () => {
      ...
      updateModel((current) => {
        ...
        const categories = adjustCurrentMonth ? current.categories.map((item) => (item.key === expenseCat ? { ...item, spentUsd: Math.max(0, item.spentUsd - usdValue) } : item)) : current.categories;
        return {
          ...current,
          expenses,
          categories,
  ...
  function deleteExpense(id: string) {
    ...
    if (!expense) return;
    const usdValue = amountUsd(expense.amount, expense.cur, model.rate);
    const adjustCurrentMonth = isExpenseTransaction(expense) && isIsoDayInBudgetCycle(expense.date, today, model.budgetCycleStartDay);
    hapticWarning();
    updateModel((current) => {
      const expenses = current.expenses.filter((item) => item.id !== id);
      const categories = adjustCurrentMonth ? current.categories.map((item) => (item.key === expense.cat ? { ...item, spentUsd: Math.max(0, item.spentUsd - usdValue) } : item)) : current.categories;
      return {
        ...current,
        expenses,
        categories,
    ...
    showToast(`Deleted ${expense.name}`, 'Undo', () => {
      hapticSelect();
      updateModel((current) => {
        const expenses = current.expenses.some((item) => item.id === expense.id) ? current.expenses : [expense, ...current.expenses];
        const categories = adjustCurrentMonth ? current.categories.map((item) => (item.key === expense.cat ? { ...item, spentUsd: item.spentUsd + usdValue } : item)) : current.categories;
        return {
          ...current,
          expenses,
          categories,
  ...
  function saveEntry() {
    ...
    const nextCat = nextKind === 'income' ? 'income' : drafts.selectedCat || selectedExpense.cat;
  ...
  function saveRecurringPayment() {
    ...
    updateModel((current) => ({
      ...
      recurringPayments: drafts.recurringEditId
        ? current.recurringPayments.map((item) => (item.id === drafts.recurringEditId ? payment : item))
        : [...current.recurringPayments, payment],
    }));
    setDrafts((current) => ({ ...current, recurringName: '', recurringAmount: '', recurringCur: 'USD', recurringDueDay: 1, recurringEditId: null }));
    setSheet(null);
    showToast(drafts.recurringEditId ? 'Recurring payment updated' : 'Recurring payment saved');
    hapticSuccess();
  }
  ...
  function deleteRecurringPayment(id: string, name: string) {
    ...
    setDrafts((current) => current.recurringEditId === id ? { ...current, recurringName: '', recurringAmount: '', recurringCur: 'USD', recurringDueDay: 1, recurringEditId: null } : current);
    showToast(`Deleted ${name}`);
    hapticWarning();
  }

  const reminderBody = todayTransactions.length === 0
  ...
  async function exportCsv() {
    const rows = [['Date', 'Type', 'Description', 'Category', 'Amount', 'Currency', 'Note']];
    model.expenses
      .filter((expense) => isoMonthFromDay(expense.date) === exportMonth)
      .forEach((expense) => {
        const kind = transactionKind(expense);
        rows.push([expense.date, kind === 'income' ? 'Income' : 'Outcome', expense.name, kind === 'income' ? 'Income' : categoryFor(model.categories, expense.cat).label, String(expense.amount), expense.cur, expense.note ?? '']);
      });
  ...
  async function importAbaStatement() {
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Could not import ABA statement');
      hapticError();
    }
  }
  ...
  function renderTopBar(title: string, _subtitle?: string, back?: Screen, right?: ReactNode) {
    return (
      <View style={styles.topRow}>
        {back ? (
          <TouchableOpacity accessibilityRole="button" accessibilityLabel="Go back" style={[styles.iconButton, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => go(back)}>
  ...
  function renderCurrencySegmentLabel(cur: Currency, selected: boolean) {
    ...
    return <AppText style={[styles.segmentText, { color: selected ? theme.text : theme.muted }]}>{cur} <AppText style={{ fontFamily: KHMER_FONT.bold }}>({symbol})</AppText></AppText>;
  }

  function renderTransactionRow(transaction: Expense, options: { showDate?: boolean; showDelete?: boolean } = {}) {
    ...
    const subtitle = [options.showDate ? transaction.date : null, isIncome ? t(model.language, 'add.income') : categoryLabel(cat, model.language), transaction.time].filter(Boolean).join(' · ');
    return (
      <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${transaction.name}, ${subtitle}, ${isIncome ? 'Income ' : ''}${amountLabel(transaction.amount, transaction.cur)}`} style={[styles.expenseRow, { borderColor: theme.line }]} onPress={() => openEntrySheet(transaction)}>
        <View style={[styles.bubble, { backgroundColor: `${color}22` }]}><Glyph name={icon} color={color} /></View>
        <View style={{ flex: 1 }}>
          <AppText style={[styles.itemName, { color: theme.text }]}>{transaction.name}</AppText>
          <AppText style={[styles.itemSub, { color: theme.muted }]}>{subtitle}</AppText>
          {transaction.note ? <AppText style={[styles.itemNote, { color: theme.faint }]}>{transaction.note}</AppText> : null}
        </View>
        <AppText style={[styles.amount, { color: isIncome ? theme.green : theme.text }]}>{isIncome ? '+' : ''}{amountLabel(transaction.amount, transaction.cur)}</AppText>
        {options.showDelete ? <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Delete ${transaction.name}`} onPress={() => deleteExpense(transaction.id)} style={{ padding: 5, marginLeft: 2 }}><MaterialIcons name="delete-outline" size={20} color={theme.faint} /></TouchableOpacity> : null}
      </TouchableOpacity>
    );
  }
    ...
    if (step === WELCOME_ONBOARDING_STEP) {
      return (
        <SafeAreaView style={[styles.safe, { backgroundColor: theme.page }]}>
          <WelcomeScreen theme={theme} onStart={() => { hapticSelect(); updateModel((current) => ({ ...current, onbStep: FIRST_SETUP_ONBOARDING_STEP })); }} />
        </SafeAreaView>
      );
    }
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.page }]}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
          <KeyboardAwareScrollView key={`setup-${step}`} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'none'} contentContainerStyle={[styles.scroll, { paddingBottom: Math.max(260, footerBottomPadding + 200) }]}>
            <View style={[styles.headerRow, { marginTop: 4, marginBottom: 20 }]}>
            <View style={{ width: 40 }}>{step > 1 ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="Previous setup step" style={[styles.iconButton, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => updateModel((current) => ({ ...current, onbStep: current.onbStep - 1 }))}><MaterialIcons name="arrow-back" size={20} color={theme.text} /></TouchableOpacity> : null}</View>
          ...
          {step === 1 ? (
            <AnimatedCue trigger={`setup-${step}`} distance={10}>
              ...
              <View style={[styles.segment, { backgroundColor: theme.surface2 }]}>
                {(['USD', 'KHR'] as Currency[]).map((cur) => <TouchableOpacity key={cur} accessibilityRole="button" accessibilityLabel={`Use ${cur === 'USD' ? 'US dollars' : 'Khmer riel'} for salary`} accessibilityState={{ selected: model.salaryCur === cur }} style={[styles.segmentButton, model.salaryCur === cur && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => { hapticSelect(); updateModel((current) => ({ ...current, salaryCur: cur })); }}>{renderCurrencySegmentLabel(cur, model.salaryCur === cur)}</TouchableOpacity>)}
              </View>
              <AppText style={[styles.help, { color: theme.muted }]}>You spend in both - set your rate anytime.</AppText>
              <AppText style={[styles.help, { color: theme.muted }]}>Right now 1 USD ≈ {khr(model.rate)}.</AppText>
            </AnimatedCue>
          ) : null}
          {step === 2 ? (
            <AnimatedCue trigger={`setup-${step}`} distance={10}>
          ...
          {step === 3 ? (
            <AnimatedCue trigger={`setup-${step}`} distance={10}>
              ...
              <BudgetMethodPicker
                ...
                custom={model.custom}
                theme={theme}
                onSelect={(mode) => { hapticSelect(); updateModel((current) => ({ ...current, method: mode })); }}
              ...
              <AnimatedSplitBar needsPct={method.needsPct} wantsPct={method.wantsPct} savePct={method.savePct} needsColor={theme.primary} wantsColor={theme.amber} saveColor={theme.green} />
  ...
  function renderHome() {
    ...
    const ringLabel = budget.status === 'over' ? t(model.language, 'home.overBudgetToday') : budget.status === 'close' ? t(model.language, 'home.almostLimit') : t(model.language, 'home.leftToday');
    ...
    const hasUpcoming = hasLoanPayment || hasRentPayment || upcomingRecurring.length > 0 || model.ious.length > 0;
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          <View style={{ flex: 1 }}><AppText style={[styles.greet, { color: theme.muted }]}>{greetingFor()}</AppText><AppText style={[styles.title, { color: theme.text }]}>{t(model.language, 'home.title')}</AppText></View>
          <TouchableOpacity accessibilityRole="switch" accessibilityLabel="Dark mode" accessibilityState={{ checked: model.dark }} style={[styles.iconButton, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => updateModel((current) => ({ ...current, dark: !current.dark }))}><MaterialIcons name={model.dark ? 'light-mode' : 'dark-mode'} size={20} color={theme.muted} /></TouchableOpacity>
        </View>
        <View style={styles.ringWrap}>
          ...
          <View style={styles.ringCenter}>
            ...
            />
            <AppText style={[styles.ringKhr, { color: theme.muted }]}>{budget.status === 'over' ? t(model.language, 'home.tomorrowAdjusts') : `≈ ${leftAlt}`}</AppText>
          </View>
        </View>
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Open current budget cycle summary" activeOpacity={0.7} style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => setSheet('month')}>
          <View style={[styles.row, { marginBottom: 16 }]}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>Current cycle</AppText><View style={[styles.duePill, { backgroundColor: theme.surface2 }]}><AppText style={[styles.duePillText, { color: theme.muted }]}>{budgetCycle.daysLeftIncludingToday} days left</AppText></View></View>
        ...
        {upcomingRecurring.map((payment) => <Upcoming key={payment.id} icon="subscriptions" color="#0ea5b7" title={payment.name} subtitle={`Recurring · ${dueText(payment.dueDay)}`} amount={amountLabel(payment.amount, payment.cur)} theme={theme} onPress={() => openRecurringSheet(payment)} onDone={() => markBillPaid(recurringPaidBillKey(payment.id), payment.name)} />)}
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Add borrowed money" style={[styles.dashed, { borderColor: theme.faint, marginTop: 8 }]} onPress={() => { setDrafts((current) => ({ ...current, iouPerson: '', iouAmount: '', iouDue: '', iouEditId: null })); setSheet('iou'); }}><MaterialIcons name="add" size={20} color={theme.primary} /><AppText style={[styles.chipText, { color: theme.primary, fontFamily: FONT.bold }]}>Borrowed money</AppText></TouchableOpacity>
        {!hasUpcoming ? <View style={{ marginTop: 14 }}><EmptyState icon="check-circle" title={t(model.language, 'home.noPaymentsTitle')} body={t(model.language, 'home.noPaymentsBody')} theme={theme} /></View> : null}
        <SectionHeader title={t(model.language, 'home.today')} action={t(model.language, 'home.summary')} theme={theme} onAction={() => go('insights')} />
        {todayTransactions.map((transaction) => {
          const row = renderTransactionRow(transaction, { showDelete: true });
          return transaction.id === recentExpenseId ? <AnimatedCue key={transaction.id} trigger={transaction.id}>{row}</AnimatedCue> : <View key={transaction.id}>{row}</View>;
        })}
        {todayTransactions.length === 0 ? <EmptyState icon="edit-note" title={t(model.language, 'home.startRecordingTitle')} body={t(model.language, 'home.startRecordingBody')} theme={theme} /> : null}
      </ScrollView>
    );
  }
  ...
  function renderAdd() {
    ...
    return (
      <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
        <KeyboardAwareScrollView key="add" extraSpace={120} contentContainerStyle={[styles.scroll, { paddingBottom: 132 }]} showsVerticalScrollIndicator={false} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'none'}>
          {renderTopBar(t(model.language, 'add.title'), undefined, 'home')}
          <AppText style={[styles.label, { color: theme.muted, marginTop: 0 }]}>{t(model.language, 'add.recordAs')}</AppText>
          <View style={[styles.segment, { backgroundColor: theme.surface2, marginBottom: 12 }]}>
            {(['expense', 'income'] as const).map((kind) => <TouchableOpacity key={kind} accessibilityRole="button" accessibilityLabel={kind === 'expense' ? 'Record as outcome' : 'Record as income'} accessibilityState={{ selected: drafts.transactionKind === kind }} style={[styles.segmentButton, drafts.transactionKind === kind && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => { hapticSelect(); setDrafts((current) => ({ ...current, transactionKind: kind, selectedCat: kind === 'income' ? '' : categoryKeyForAddText(current.addText) })); }}><AppText style={[styles.segmentText, { color: drafts.transactionKind === kind ? theme.text : theme.muted }]}>{kind === 'expense' ? t(model.language, 'add.outcome') : t(model.language, 'add.income')}</AppText></TouchableOpacity>)}
          </View>
          <TextInput style={[styles.addInput, { backgroundColor: theme.surface, borderColor: theme.line, color: theme.text }]} placeholder={drafts.transactionKind === 'income' ? t(model.language, 'add.tryIncome') : t(model.language, 'add.tryExpense')} placeholderTextColor={theme.faint} value={drafts.addText} onChangeText={(value) => setDrafts((current) => ({ ...current, addText: value, selectedCat: current.transactionKind === 'income' ? '' : categoryKeyForAddText(value) }))} />
          <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.line }]}>
            <View style={styles.aiHeader}><MaterialIcons name="auto-awesome" size={17} color={drafts.transactionKind === 'income' ? theme.green : theme.primary} /><AppText style={[styles.aiHeaderText, { color: drafts.transactionKind === 'income' ? theme.green : theme.primary }]}>{t(model.language, 'add.understood')}</AppText></View>
            <View style={[styles.headerRow, { alignItems: 'flex-end', marginTop: 6 }]}>
              <View style={{ flex: 1 }}>
                ...
                <AnimatedCue trigger={hasAmount ? `${parsedAmount?.currency}-${parsedAmount?.amountMinor}` : 'empty'} distance={4} scale={1}>
                  <AppText style={[styles.itemSub, { color: theme.muted }]}>{parsedAmount ? parsedAmount.currency === 'KHR' ? `≈ ${usd((parsedAmount.amountMinor / model.rate))}` : `≈ ${khr((parsedAmount.amountMinor / 100) * model.rate)}` : t(model.language, 'add.startTyping')}</AppText>
                </AnimatedCue>
              </View>
              <AnimatedCue trigger={drafts.transactionKind === 'income' ? 'income' : parsedCat.key} distance={4}>
                <View style={[styles.bubbleLarge, { backgroundColor: drafts.transactionKind === 'income' ? `${theme.green}22` : `${parsedCat.color}22` }]}><Glyph name={drafts.transactionKind === 'income' ? 'account-balance-wallet' : parsedCat.icon} size={27} color={drafts.transactionKind === 'income' ? theme.green : parsedCat.color} /></View>
              </AnimatedCue>
            </View>
            {drafts.transactionKind === 'income' ? <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 16 }]}>{t(model.language, 'add.incomeExplainer')}</AppText> : <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 16 }]}>{t(model.language, 'add.categoryTap')}</AppText>}
          </View>
          <AppText style={[styles.label, { color: theme.muted }]}>Quick amount</AppText>
          <View style={styles.chipRow}>
            {QUICK_AMOUNT_CHIPS.map((chip) => <TouchableOpacity key={chip.label} accessibilityRole="button" accessibilityLabel={`Use quick amount ${chip.label}`} style={[styles.chip, { borderColor: theme.line, backgroundColor: chip.cur === 'KHR' ? theme.primaryWash : theme.surface }]} onPress={() => { hapticSelect(); setDrafts((current) => { const addText = applyQuickAmountChip(current.addText, chip.text); return { ...current, addText, selectedCat: current.transactionKind === 'income' ? '' : categoryKeyForAddText(addText) }; }); }}><AppText style={[styles.chipText, { color: chip.cur === 'KHR' ? theme.primary : theme.text }]}>{chip.label}</AppText></TouchableOpacity>)}
          </View>
          {drafts.transactionKind === 'expense' ? <View style={styles.chipRow}>{model.categories.map((cat) => { const on = drafts.selectedCat === cat.key; return <SelectableChip key={cat.key} icon={cat.icon} label={categoryLabel(cat, model.language)} selected={on} selectedColor={cat.color} textColor="#fff" mutedColor={theme.muted} borderColor={theme.line} surfaceColor={theme.surface} onPress={() => { hapticSelect(); setDrafts((current) => ({ ...current, selectedCat: cat.key })); }} />; })}</View> : null}
          {drafts.transactionKind === 'expense' && model.categories.length === 0 ? <EmptyState icon="category" title={t(model.language, 'add.noCategoriesTitle')} body={t(model.language, 'add.noCategoriesBody')} theme={theme} /> : null}
          <AppText style={[styles.label, { color: theme.muted }]}>{t(model.language, 'add.transactionDate')}</AppText>
          <TextInput style={[styles.input, { backgroundColor: theme.surface, borderColor: hasValidDate ? theme.line : theme.red, color: theme.text, fontFamily: FONT.semibold, fontSize: 16 }]} placeholder="YYYY-MM-DD" placeholderTextColor={theme.faint} value={drafts.expenseDate} onChangeText={(value) => setDrafts((current) => ({ ...current, expenseDate: value }))} autoCapitalize="none" autoCorrect={false} returnKeyType="next" />
          {!hasValidDate ? <AppText style={[styles.itemSub, { color: theme.red, marginTop: 7 }]}>{t(model.language, 'add.dateFormatHelp', { date: today })}</AppText> : null}
          <AppText style={[styles.label, { color: theme.muted }]}>{t(model.language, 'add.noteOptional')}</AppText>
          <TextInput style={[styles.input, { backgroundColor: theme.surface, borderColor: theme.line, color: theme.text, fontFamily: FONT.semibold, fontSize: 16 }]} placeholder={t(model.language, 'add.notePlaceholder')} placeholderTextColor={theme.faint} value={drafts.addNote} onChangeText={(value) => setDrafts((current) => ({ ...current, addNote: value }))} returnKeyType="done" />
        </KeyboardAwareScrollView>
        <View style={[styles.footer, { backgroundColor: theme.page, borderColor: theme.line }]}>
          <AnimatedPressable disabled={!canSave} accessibilityRole="button" accessibilityLabel={parsedAmount ? `${t(model.language, 'add.button')} ${drafts.transactionKind === 'income' ? t(model.language, 'add.income') : formatMoney(parsedAmount)}` : t(model.language, 'add.title')} style={[styles.button, { backgroundColor: theme.primary, opacity: canSave ? 1 : 0.35 }]} contentStyle={styles.buttonContent} onPress={addExpense}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText numberOfLines={1} style={styles.buttonText}>{t(model.language, 'add.button')} {parsedAmount ? drafts.transactionKind === 'income' ? `${t(model.language, 'add.income')} ${formatMoney(parsedAmount)}` : formatMoney(parsedAmount) : ''}</AppText></AnimatedPressable>
        </View>
      </KeyboardAvoidingView>
    );
  }
  ...
  function renderCategories() {
    const visibleCategories = model.categories.filter((cat) => cat.budgetUsd > 0);
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {renderTopBar(t(model.language, 'categories.title'), t(model.language, 'categories.subtitle'))}
        <View style={styles.grid}>{visibleCategories.map((cat) => {
          ...
          return <TouchableOpacity key={cat.key} accessibilityRole="button" accessibilityLabel={`Open ${label} category`} style={[styles.catCard, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => { setDrafts((current) => ({ ...current, selectedCat: cat.key })); go('detail'); }}><View style={styles.row}><View style={[styles.bubble, { backgroundColor: `${cat.color}22` }]}><Glyph name={cat.icon} color={cat.color} /></View><MaterialIcons name="chevron-right" size={20} color={theme.faint} /></View><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14, marginTop: 16 }]}>{label}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{usd(spentUsd)} / {usd0(cat.budgetUsd)}</AppText><Progress label={`${pct}%`} value={usd(spentUsd)} total={usd0(cat.budgetUsd)} pct={cat.budgetUsd > 0 ? spentUsd / cat.budgetUsd : 0} color={over ? theme.red : cat.color} theme={theme} /></TouchableOpacity>;
        })}</View>
        {visibleCategories.length === 0 ? <EmptyState icon="category" title={t(model.language, 'categories.emptyTitle')} body={t(model.language, 'categories.emptyBody')} theme={theme} /> : null}
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={t(model.language, 'categories.addCategory')} style={[styles.dashed, { borderColor: theme.faint }]} onPress={() => openCatSheet(null)}><MaterialIcons name="add" size={20} color={theme.primary} /><AppText style={[styles.chipText, { color: theme.primary, fontFamily: FONT.bold }]}>{t(model.language, 'categories.addCategory')}</AppText></TouchableOpacity>
      </ScrollView>
    );
  }
  ...
  function renderDetail() {
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {renderTopBar(selectedCategoryLabel, undefined, 'categories', <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Edit ${selectedCategoryLabel} category`} style={[styles.iconButton, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => openCatSheet(selectedCategory.key)}><MaterialIcons name="edit" size={19} color={theme.muted} /></TouchableOpacity>)}
        <View style={[styles.card, styles.row, { backgroundColor: theme.surface, borderColor: theme.line, marginBottom: 0 }]}><View style={{ flex: 1 }}><AppText style={[styles.label, { color: theme.muted, marginTop: 0, marginBottom: 6 }]}>{t(model.language, 'categories.spentThisMonth')}</AppText><AppText style={[styles.parsedAmount, { color: theme.text, fontSize: 32, marginTop: 0 }]}>{usd(categoryMonthSpentUsd(selectedCategory.key))}</AppText><AppText style={[styles.itemSub, { color: theme.muted, marginTop: 2 }]}>of {usd0(selectedCategory.budgetUsd)} {t(model.language, 'categories.budgetSuffix')}</AppText></View><View style={[styles.bubbleLarge, { backgroundColor: `${selectedCategory.color}22`, width: 56, height: 56, borderRadius: 18 }]}><Glyph name={selectedCategory.icon} size={30} color={selectedCategory.color} /></View></View>
        <SectionHeader title={t(model.language, 'categories.transactions')} theme={theme} />
        {txns.map((expense) => <TouchableOpacity key={expense.id} accessibilityRole="button" accessibilityLabel={`Edit ${expense.name}, ${expense.date}, ${amountLabel(expense.amount, expense.cur)}`} style={[styles.expenseRow, { borderColor: theme.line }]} onPress={() => openEntrySheet(expense)}><View style={[styles.bubble, { backgroundColor: `${selectedCategory.color}22` }]}><Glyph name={selectedCategory.icon} color={selectedCategory.color} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemName, { color: theme.text }]}>{expense.name}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{expense.date} · {expense.time}</AppText>{expense.note ? <AppText style={[styles.itemNote, { color: theme.faint }]}>{expense.note}</AppText> : null}</View><AppText style={[styles.amount, { color: theme.text }]}>{amountLabel(expense.amount, expense.cur)}</AppText></TouchableOpacity>)}
        {txns.length === 0 ? <EmptyState icon="receipt-long" title={t(model.language, 'categories.noTransactionsTitle')} body={t(model.language, 'categories.noTransactionsBody')} theme={theme} /> : null}
      </ScrollView>
    );
  }
  ...
  function renderGoals() {
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} scrollEnabled={draggingGoal === null}>
        ...
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="New goal" style={[styles.dashed, { borderColor: theme.faint }]} onPress={() => setSheet('goal')}><MaterialIcons name="add" size={20} color={theme.primary} /><AppText style={[styles.chipText, { color: theme.primary, fontFamily: FONT.bold }]}>New goal</AppText></TouchableOpacity>
      </ScrollView>
    );
  }
  ...
  function renderInsights() {
    ...
    const weekLabels = weekItems.map((item) => new Date(`${item.date}T00:00:00`).toLocaleDateString('en-US', { weekday: 'short' }).slice(0, 1));
    const chartH = 120;
    ...
    const dayLabel = (index: number) => {
      ...
      return date ? new Date(`${date}T00:00:00`).toLocaleDateString('en-US', { weekday: 'short' }) : 'Day';
    };
    ...
    const insightTip = topCategory && topCategory.spentUsd > 0 ? `${topCategory.category.label} is your top spend this budget cycle. Small cuts there will move your savings fastest.` : 'Add expenses today to see category-specific savings tips.';
    const exportMonths = [0, -1, -2].map((delta) => shiftIsoMonth(thisMonth, delta));
    const insightDays = new Set(monthTransactions.map((expense) => expense.date)).size;
  ...
  function renderSettings() {
    ...
    const rows: readonly [string, string, Sheet | 'categories', IconName][] = [
      [t(model.language, 'settings.monthlyIncome'), `${amountLabel(model.salary, model.salaryCur)} / month`, 'income', 'account-balance-wallet'],
      [t(model.language, 'settings.rentUtilities'), model.rent + model.utilities > 0 ? `${usd(model.rent + model.utilities)} · due ${ordinal(model.rentDue)}` : t(model.language, 'settings.notSet'), 'fixed', 'receipt-long'],
      [t(model.language, 'settings.loanRepayment'), model.loan > 0 ? `${usd(model.loan)} · due ${ordinal(model.loanDue)}` : t(model.language, 'settings.noLoan'), 'loan', 'savings'],
      [t(model.language, 'settings.budgetingMethod'), model.method === 'balanced' ? 'Balanced (50/30/20)' : model.method === 'saver' ? 'Saver (40/30/30)' : `Custom (${model.custom.needs}/${model.custom.wants}/${model.custom.save})`, 'method', 'pie-chart'],
      ['Budget cycle', `Starts ${ordinal(model.budgetCycleStartDay)} · ${budgetCycleLabel}`, 'cycle', 'calendar-month'],
      [t(model.language, 'settings.currencies'), `Auto daily USD/KHR · $1 = ${khr(model.rate)}\n${rateCheckLabel}`, 'currency', 'currency-exchange'],
      [t(model.language, 'settings.categories'), t(model.language, 'settings.categoriesCount', { count: model.categories.length }), 'categories', 'grid-view'],
      ['Recurring payments', model.recurringPayments.length > 0 ? `${model.recurringPayments.length} saved` : 'Subscriptions and other monthly payments', 'recurring', 'subscriptions'],
      [t(model.language, 'settings.dailyReminder'), formatClock(model.notify), 'reminder', 'schedule'],
    ];
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {renderTopBar(t(model.language, 'settings.title'), t(model.language, 'settings.subtitle'))}
        {rows.map((row) => <TouchableOpacity key={row[0]} accessibilityRole="button" accessibilityLabel={`${row[0]}, ${row[1]}`} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={() => (row[2] === 'categories' ? go('categories') : setSheet(row[2]))}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name={row[3]} size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{row[0]}</AppText><AppText style={[styles.itemSub, { color: theme.muted, lineHeight: 18 }]}>{row[1]}</AppText></View><MaterialIcons name="chevron-right" size={22} color={theme.faint} /></TouchableOpacity>)}
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${t(model.language, 'settings.language')}, ${LANGUAGE_LABELS[model.language]}`} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={() => updateModel((current) => ({ ...current, language: current.language === 'en' ? 'km' : 'en' }))}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name="language" size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{t(model.language, 'settings.language')}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{LANGUAGE_LABELS[model.language]} · {t(model.language, 'settings.languageDetail')}</AppText></View><AppText style={[styles.itemSub, { color: theme.primary, fontFamily: FONT.bold }]}>{LANGUAGE_LABELS[nextLanguage]}</AppText></TouchableOpacity>
        <TouchableOpacity accessibilityRole="switch" accessibilityLabel={t(model.language, 'settings.billReminders')} accessibilityState={{ checked: model.billReminders }} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={() => updateModel((current) => ({ ...current, billReminders: !current.billReminders }))}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name="notifications" size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{t(model.language, 'settings.billReminders')}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{model.billReminders ? t(model.language, 'settings.on') : t(model.language, 'settings.off')}</AppText></View><MaterialIcons name={model.billReminders ? 'toggle-on' : 'toggle-off'} size={30} color={model.billReminders ? theme.primary : theme.faint} /></TouchableOpacity>
        <TouchableOpacity accessibilityRole="switch" accessibilityLabel="Dark mode" accessibilityState={{ checked: model.dark }} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={() => updateModel((current) => ({ ...current, dark: !current.dark }))}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name={model.dark ? 'dark-mode' : 'light-mode'} size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{t(model.language, 'settings.appearance')}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{model.dark ? t(model.language, 'settings.darkMode') : t(model.language, 'settings.lightMode')}</AppText></View><MaterialIcons name={model.dark ? 'toggle-on' : 'toggle-off'} size={30} color={model.dark ? theme.primary : theme.faint} /></TouchableOpacity>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={t(model.language, 'settings.exportJsonBackup')} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={exportJsonBackup}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name="file-download" size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{t(model.language, 'settings.exportJsonBackup')}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{t(model.language, 'settings.exportDescription')}</AppText></View><MaterialIcons name="ios-share" size={22} color={theme.faint} /></TouchableOpacity>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={t(model.language, 'settings.importJsonBackup')} style={[styles.settingsRow, { borderColor: theme.line }]} onPress={importJsonBackup}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name="file-upload" size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>{t(model.language, 'settings.importJsonBackup')}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{t(model.language, 'settings.importDescription')}</AppText></View><MaterialIcons name="chevron-right" size={22} color={theme.faint} /></TouchableOpacity>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Import ABA statement" style={[styles.settingsRow, { borderColor: theme.line }]} onPress={importAbaStatement}><View style={[styles.settingsIcon, { backgroundColor: theme.surface2 }]}><MaterialIcons name="account-balance" size={21} color={theme.muted} /></View><View style={{ flex: 1 }}><AppText style={[styles.itemTitle, { color: theme.text, fontSize: 14 }]}>Import ABA statement</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>Add transactions from ABA CSV or Excel export</AppText></View><MaterialIcons name="chevron-right" size={22} color={theme.faint} /></TouchableOpacity>
        <View style={{ alignItems: 'center', marginTop: 32, gap: 6, opacity: 0.7 }}><View style={styles.brandRow}><Image source={require('./assets/luy-khnom-favicon-flat.png')} style={styles.footerLogoImage} resizeMode="contain" /><AppText style={[styles.brandName, { color: theme.text, fontSize: 14 }]}>Luy Khnom</AppText></View><AppText style={[styles.itemSub, { color: theme.muted }]}>{t(model.language, 'app.version')}</AppText></View>
      </ScrollView>
    );
  }
  ...
  function renderSheet() {
    return (
      <Modal visible={sheet !== null} transparent animationType="slide" statusBarTranslucent navigationBarTranslucent onRequestClose={() => setSheet(null)}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
          <Pressable style={styles.scrim} onPress={() => setSheet(null)}>
            ...
            <View style={[styles.headerRow, { marginBottom: 4 }]}><AppText style={[styles.sheetTitle, { color: theme.text }]}>{sheet === 'category' && drafts.categoryEditKey ? 'Edit category' : sheetTitle(sheet)}</AppText><TouchableOpacity accessibilityRole="button" accessibilityLabel="Close sheet" style={[styles.iconButton, { backgroundColor: theme.surface, borderColor: theme.line }]} onPress={() => setSheet(null)}><MaterialIcons name="close" size={20} color={theme.muted} /></TouchableOpacity></View>
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'none'} contentContainerStyle={{ paddingBottom: sheet === 'category' ? 36 : sheet === 'month' || sheet === 'day' ? 44 : 8 }}>
            {sheet === 'income' ? <View><SheetInput label="Monthly salary" value={String(model.salary)} theme={theme} onChange={(value) => updateModel((current) => ({ ...current, salary: Number(value) || 0 }))} /><AppText style={[styles.itemSub, { color: theme.muted, marginTop: 7 }]}>{model.salaryCur === 'KHR' ? `≈ ${usd(model.salary / model.rate)}` : `≈ ${khr(model.salary * model.rate)}`}</AppText><AppText style={[styles.label, { color: theme.muted }]}>Currency</AppText><View style={[styles.segment, { backgroundColor: theme.surface2 }]}>{(['USD', 'KHR'] as Currency[]).map((cur) => <TouchableOpacity key={cur} accessibilityRole="button" accessibilityLabel={`Use ${cur === 'USD' ? 'US dollars' : 'Khmer riel'} for salary`} accessibilityState={{ selected: model.salaryCur === cur }} style={[styles.segmentButton, model.salaryCur === cur && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => updateModel((current) => ({ ...current, salaryCur: cur }))}>{renderCurrencySegmentLabel(cur, model.salaryCur === cur)}</TouchableOpacity>)}</View></View> : null}
            ...
            {sheet === 'cycle' ? <View>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 8, lineHeight: 21 }]}>Start the monthly budget on payday instead of the 1st.</AppText>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>For a salary paid on the 5th, choose day 5.</AppText>
              <DueDayPicker label="Cycle starts each month" value={model.budgetCycleStartDay} theme={theme} onChange={(day) => updateModel((current) => ({ ...current, budgetCycleStartDay: day, lastActiveMonth: budgetCycleKeyForDay(today, day) }))} />
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>Current cycle: {budgetCycleLabel}.</AppText>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>Income, spending, days left, and category totals use this range.</AppText>
            </View> : null}
            {sheet === 'currency' ? <View>
              <AppText style={[styles.label, { color: theme.muted }]}>Base currency — used for the home budget display</AppText>
              <View style={[styles.segment, { backgroundColor: theme.surface2 }]}>{(['USD', 'KHR'] as Currency[]).map((cur) => <TouchableOpacity key={cur} accessibilityRole="button" accessibilityLabel={`Use ${cur === 'USD' ? 'US dollars' : 'Khmer riel'} as base currency`} accessibilityState={{ selected: model.baseCur === cur }} style={[styles.segmentButton, model.baseCur === cur && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => updateModel((current) => ({ ...current, baseCur: cur }))}>{renderCurrencySegmentLabel(cur, model.baseCur === cur)}</TouchableOpacity>)}</View>
              <SheetInput label="Conversion rate — KHR per $1" value={String(model.rate)} theme={theme} onChange={(value) => updateModel((current) => { const nextRate = Number(value); return { ...current, rate: Number.isFinite(nextRate) && nextRate > 0 ? round2(nextRate) : RATE, exchangeRateSource: 'Manual override' }; })} />
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>Auto-checks once per day from {USD_TO_KHR_RATE_SOURCE.name}.</AppText>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>If you’re offline or the data is missing, Luy Khnom keeps this value.</AppText>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 8, lineHeight: 21 }]}>{model.exchangeRateLastFetchedDay ? `Last checked ${model.exchangeRateLastFetchedDay}` : 'Not checked yet'}{'\n'}Current source: {model.exchangeRateSource === USD_TO_KHR_RATE_SOURCE.name ? 'Rates from Exchange Rate.' : `${model.exchangeRateSource}.`}</AppText>
            </View> : null}
            ...
            {sheet === 'category' ? <View>
              ...
              <View style={styles.iconGrid}>
                {ICON_SET.map((ic) => { const on = drafts.categoryIcon === ic; return <TouchableOpacity key={ic} accessibilityRole="button" accessibilityLabel={`Choose ${ic} icon`} accessibilityState={{ selected: on }} style={[styles.iconPick, { borderColor: on ? drafts.categoryColor : theme.line, backgroundColor: on ? `${drafts.categoryColor}22` : theme.surface }]} onPress={() => setDrafts((current) => ({ ...current, categoryIcon: ic }))}><Glyph name={ic} size={22} color={on ? drafts.categoryColor : theme.muted} /></TouchableOpacity>; })}
                {Array.from({ length: (6 - (ICON_SET.length % 6)) % 6 }).map((_, index) => <View key={`icon-spacer-${index}`} style={styles.iconPickSpacer} />)}
              </View>
              <AppText style={[styles.label, { color: theme.muted }]}>Color</AppText>
              <View style={styles.colorGrid}>{PALETTE.map((color) => <TouchableOpacity key={color} accessibilityRole="button" accessibilityLabel={`Choose color ${color}`} accessibilityState={{ selected: drafts.categoryColor === color }} style={[styles.swatch, { backgroundColor: color, borderColor: drafts.categoryColor === color ? theme.text : 'transparent' }]} onPress={() => setDrafts((current) => ({ ...current, categoryColor: color }))} />)}</View>
              <TouchableOpacity accessibilityRole="button" accessibilityLabel={drafts.categoryEditKey ? "Save category changes" : "Add category"} style={[styles.button, { backgroundColor: theme.primary, marginTop: 22 }]} onPress={saveCategory}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>{drafts.categoryEditKey ? 'Save changes' : 'Add category'}</AppText></TouchableOpacity>
            </View> : null}
            {sheet === 'entry' && selectedExpense ? <View>
              <AppText style={[styles.label, { color: theme.muted, marginTop: 10 }]}>Record as</AppText>
              <View style={[styles.segment, { backgroundColor: theme.surface2 }]}> 
                {(['expense', 'income'] as const).map((kind) => <TouchableOpacity key={kind} accessibilityRole="button" accessibilityLabel={kind === 'expense' ? 'Record as outcome' : 'Record as income'} accessibilityState={{ selected: drafts.transactionKind === kind }} style={[styles.segmentButton, drafts.transactionKind === kind && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => { hapticSelect(); setDrafts((current) => ({ ...current, transactionKind: kind, selectedCat: kind === 'income' ? '' : current.selectedCat })); }}><AppText style={[styles.segmentText, { color: drafts.transactionKind === kind ? theme.text : theme.muted }]}>{kind === 'expense' ? 'Outcome' : 'Income'}</AppText></TouchableOpacity>)}
              </View>
              <MoneyField label="Amount" initialAmount={Number(drafts.iouAmount) || 0} initialCur={drafts.entryCur} rate={model.rate} theme={theme} onChange={(_usd, amount, cur) => setDrafts((current) => ({ ...current, iouAmount: amount ? String(amount) : '', entryCur: cur }))} />
              <SheetInput label="Transaction date (YYYY-MM-DD)" value={drafts.expenseDate} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, expenseDate: value }))} />
              {!isIsoDay(drafts.expenseDate.trim()) ? <AppText style={[styles.itemSub, { color: theme.red, marginTop: 7 }]}>Use YYYY-MM-DD, for example {today}.</AppText> : null}
              <SheetInput label="Remark (optional)" value={drafts.addNote} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, addNote: value }))} />
              {drafts.transactionKind === 'expense' ? <><AppText style={[styles.label, { color: theme.muted }]}>Category</AppText><View style={styles.chipRow}>{model.categories.map((cat) => { const on = drafts.selectedCat === cat.key; return <TouchableOpacity key={cat.key} accessibilityRole="button" accessibilityLabel={`Select ${cat.label} category`} accessibilityState={{ selected: on }} style={[styles.chip, { borderColor: on ? cat.color : theme.line, backgroundColor: on ? cat.color : theme.surface }]} onPress={() => setDrafts((current) => ({ ...current, selectedCat: cat.key }))}><Glyph name={cat.icon} size={18} color={on ? '#fff' : cat.color} /><AppText style={[styles.chipText, { color: on ? '#fff' : theme.muted }]}>{cat.label}</AppText></TouchableOpacity>; })}</View></> : <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 12 }]}>Income is kept in transactions and added to this month’s available budget.</AppText>}
              <TouchableOpacity disabled={!isIsoDay(drafts.expenseDate.trim())} accessibilityRole="button" accessibilityLabel="Save entry" accessibilityState={{ disabled: !isIsoDay(drafts.expenseDate.trim()) }} style={[styles.button, { backgroundColor: theme.primary, opacity: isIsoDay(drafts.expenseDate.trim()) ? 1 : 0.35, marginTop: 22 }]} onPress={saveEntry}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>Save entry</AppText></TouchableOpacity>
            </View> : null}
            {sheet === 'iou' ? <View><SheetInput label="Who did you borrow from?" value={drafts.iouPerson} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, iouPerson: value }))} /><MoneyField label="Amount" initialAmount={Number(drafts.iouAmount) || 0} rate={model.rate} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, iouAmount: value ? String(value) : '' }))} /><SheetInput label="Pay back by" value={drafts.iouDue} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, iouDue: value }))} /><TouchableOpacity accessibilityRole="button" accessibilityLabel="Save borrowed money" style={[styles.button, { backgroundColor: theme.primary, marginTop: 22 }]} onPress={saveIou}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>Save</AppText></TouchableOpacity></View> : null}
            {sheet === 'goal' ? <View><SheetInput label="What are you saving for?" value={drafts.goalName} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, goalName: value }))} /><MoneyField label="Target amount" initialAmount={Number(drafts.goalTarget) || 0} rate={model.rate} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, goalTarget: value ? String(value) : '' }))} /><MoneyField label="Save per month" initialAmount={Number(drafts.goalPer) || 0} rate={model.rate} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, goalPer: value ? String(value) : '' }))} /><TouchableOpacity accessibilityRole="button" accessibilityLabel="Save goal" style={[styles.button, { backgroundColor: theme.primary, marginTop: 22 }]} onPress={saveGoal}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>Create goal</AppText></TouchableOpacity></View> : null}
            {sheet === 'recurring' ? <View>
              {model.recurringPayments.length > 0 ? <View>
                <AppText style={[styles.label, { color: theme.muted, marginTop: 10 }]}>Saved payments</AppText>
                {model.recurringPayments.map((payment) => (
                  <View key={payment.id} style={[styles.expenseRow, { borderColor: theme.line }]}>
                    <View style={[styles.bubble, { backgroundColor: `${theme.primary}22` }]}><Glyph name="subscriptions" color={theme.primary} /></View>
                    <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Edit ${payment.name}`} style={{ flex: 1 }} onPress={() => openRecurringSheet(payment)}>
                      <AppText style={[styles.itemName, { color: theme.text }]}>{payment.name}</AppText>
                      <AppText style={[styles.itemSub, { color: theme.muted }]}>{dueText(payment.dueDay)} · {amountLabel(payment.amount, payment.cur)}</AppText>
                    </TouchableOpacity>
                    <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Delete ${payment.name}`} onPress={() => deleteRecurringPayment(payment.id, payment.name)} style={{ padding: 5, marginLeft: 4 }}><MaterialIcons name="delete-outline" size={20} color={theme.faint} /></TouchableOpacity>
                  </View>
                ))}
              </View> : <EmptyState icon="subscriptions" title="No recurring payments yet" body="Add subscriptions or other monthly payments you want to see in Coming up." theme={theme} />}
              <View key={drafts.recurringEditId ?? 'new-recurring'}>
                <SheetInput label="Payment name" value={drafts.recurringName} theme={theme} onChange={(value) => setDrafts((current) => ({ ...current, recurringName: value }))} />
                <MoneyField label="Amount" initialAmount={Number(drafts.recurringAmount) || 0} initialCur={drafts.recurringCur} rate={model.rate} theme={theme} onChange={(_usd, amount, cur) => setDrafts((current) => ({ ...current, recurringAmount: amount ? String(amount) : '', recurringCur: cur }))} />
                <DueDayPicker label="Due each month" value={drafts.recurringDueDay} theme={theme} onChange={(day) => setDrafts((current) => ({ ...current, recurringDueDay: day }))} />
                <TouchableOpacity accessibilityRole="button" accessibilityLabel={drafts.recurringEditId ? 'Save recurring payment changes' : 'Add recurring payment'} style={[styles.button, { backgroundColor: theme.primary, marginTop: 22 }]} onPress={saveRecurringPayment}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>{drafts.recurringEditId ? 'Save changes' : 'Add payment'}</AppText></TouchableOpacity>
              </View>
            </View> : null}
            {sheet === 'month' ? <View>
              <View style={[styles.row, { marginTop: 10 }]}><AppText style={[styles.itemSub, { color: theme.muted }]}>Spent {formatMoney(budget.spentMonthUsd)} of {formatMoney0(budget.spendableMonthUsd)}</AppText><AppText style={[styles.itemSub, { color: theme.muted }]}>{filteredTransactions.length} of {transactionHistoryBase.length}</AppText></View>
              {monthIncomeUsd > 0 && transactionMonthFilter === 'current-cycle' ? <AppText style={[styles.itemSub, { color: theme.green, marginBottom: 8 }]}>Income recorded this cycle: +{usd(monthIncomeUsd)}</AppText> : null}
              <TextInput style={[styles.input, { backgroundColor: theme.surface, borderColor: theme.line, color: theme.text, fontFamily: FONT.semibold, fontSize: 15 }]} placeholder="Search name, note, category, or type" placeholderTextColor={theme.faint} value={drafts.transactionSearch} onChangeText={(value) => setDrafts((current) => ({ ...current, transactionSearch: value }))} autoCapitalize="none" autoCorrect={false} />
              <AppText style={[styles.label, { color: theme.muted }]}>Type</AppText>
              <View style={[styles.segment, { backgroundColor: theme.surface2 }]}>
                {(['all', 'expense', 'income'] as const).map((kind) => <TouchableOpacity key={kind} accessibilityRole="button" accessibilityLabel={`Show ${kind === 'all' ? 'all transactions' : kind === 'expense' ? 'outcomes' : 'income'}`} accessibilityState={{ selected: drafts.transactionTypeFilter === kind }} style={[styles.segmentButton, drafts.transactionTypeFilter === kind && { backgroundColor: theme.surface, ...CARD_SHADOW }]} onPress={() => setDrafts((current) => ({ ...current, transactionTypeFilter: kind, transactionCategoryFilter: kind === 'income' ? 'income' : current.transactionCategoryFilter === 'income' ? '' : current.transactionCategoryFilter }))}><AppText style={[styles.segmentText, { color: drafts.transactionTypeFilter === kind ? theme.text : theme.muted }]}>{kind === 'all' ? 'All' : kind === 'expense' ? 'Outcome' : 'Income'}</AppText></TouchableOpacity>)}
              </View>
              <AppText style={[styles.label, { color: theme.muted }]}>Period</AppText>
              <View style={styles.chipRow}>
                {transactionHistoryMonths.map((month) => { const on = transactionMonthFilter === month; const label = month === 'current-cycle' ? 'Current cycle' : monthLabel(month); return <TouchableOpacity key={month} accessibilityRole="button" accessibilityLabel={`Filter by ${label}`} accessibilityState={{ selected: on }} style={[styles.chip, { borderColor: on ? theme.primary : theme.line, backgroundColor: on ? theme.primary : theme.surface }]} onPress={() => setDrafts((current) => ({ ...current, transactionMonth: month }))}><AppText style={[styles.chipText, { color: on ? '#fff' : theme.muted }]}>{label}</AppText></TouchableOpacity>; })}
              </View>
              <AppText style={[styles.label, { color: theme.muted }]}>Category</AppText>
              <View style={styles.chipRow}>
                {[{ key: '', label: 'All', color: theme.primary, icon: 'all-inclusive' as IconName }, { key: 'income', label: 'Income', color: theme.green, icon: 'account-balance-wallet' as IconName }, ...model.categories].map((cat) => { const on = drafts.transactionCategoryFilter === cat.key; return <TouchableOpacity key={cat.key || 'all'} accessibilityRole="button" accessibilityLabel={`Filter by ${cat.label}`} accessibilityState={{ selected: on }} style={[styles.chip, { borderColor: on ? cat.color : theme.line, backgroundColor: on ? cat.color : theme.surface }]} onPress={() => setDrafts((current) => ({ ...current, transactionCategoryFilter: cat.key, transactionTypeFilter: cat.key === 'income' ? 'income' : current.transactionTypeFilter === 'income' && cat.key ? 'expense' : current.transactionTypeFilter }))}><Glyph name={cat.icon} size={18} color={on ? '#fff' : cat.color} /><AppText style={[styles.chipText, { color: on ? '#fff' : theme.muted }]}>{cat.label}</AppText></TouchableOpacity>; })}
              </View>
              {transactionHistoryFilterActive ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="Clear transaction filters" style={[styles.dashed, { borderColor: theme.faint, marginTop: 14 }]} onPress={() => setDrafts((current) => ({ ...current, transactionSearch: '', transactionTypeFilter: 'all', transactionCategoryFilter: '', transactionMonth: 'current-cycle' }))}><MaterialIcons name="filter-alt-off" size={20} color={theme.primary} /><AppText style={[styles.chipText, { color: theme.primary, fontFamily: FONT.bold }]}>Clear filters</AppText></TouchableOpacity> : null}
              {filteredTransactions.map((transaction) => <View key={transaction.id}>{renderTransactionRow(transaction, { showDate: true })}</View>)}
              {filteredTransactions.length === 0 ? <View style={{ marginTop: 20 }}><EmptyState icon="calendar-month" title="No matching transactions" body="Try a different search, type, category, or period filter." theme={theme} /></View> : null}
            </View> : null}
            {sheet === 'day' ? <View>
              <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.line }]}><AppText style={[styles.label, { color: theme.muted, marginTop: 0 }]}>Spent on {selectedDayDate}</AppText><AppText style={[styles.parsedAmount, { color: theme.text, fontSize: 26 }]}>{usd(selectedDayItem.value)}</AppText></View>
              {selectedDayTransactions.map((transaction) => <View key={transaction.id}>{renderTransactionRow(transaction)}</View>)}
              {selectedDayTransactions.length === 0 ? <EmptyState icon="receipt-long" title="No transactions this day" body="Income and outcomes saved for this date will show up here." theme={theme} /> : null}
            </View> : null}
            {sheet === 'formula' ? <View>
              ...
              <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.line, marginTop: 14 }]}>
                <Row label="Cycle income" value={formatMoney0(budget.salaryUsd)} theme={theme} />
                ...
                <View style={{ height: 1, backgroundColor: theme.line, marginVertical: 10 }} />
                <Row label="Free this cycle" value={formatMoney0(budget.spendableMonthUsd)} theme={theme} strong />
              ...
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>Rollover uses unused safe-to-spend from yesterday.</AppText>
              <AppText style={[styles.itemSub, { color: theme.muted, marginTop: 10, lineHeight: 21 }]}>Overspending reduces tomorrow instead of hiding the debt.</AppText>
            </View> : null}
            {sheet && ['income', 'fixed', 'loan', 'method', 'cycle', 'currency', 'reminder', 'formula'].includes(sheet) ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="Done" style={[styles.button, { backgroundColor: theme.primary, marginTop: 22 }]} onPress={() => setSheet(null)}><MaterialIcons name="check-circle" size={20} color="#fff" /><AppText style={styles.buttonText}>Done</AppText></TouchableOpacity> : null}
              </ScrollView>
            </Pressable>
          </Pressable>
        </KeyboardAvoidingView>
  ...
  if (!hydrated || !fontsLoaded) {
    screen = <SafeAreaView style={[styles.safe, { backgroundColor: theme.page, alignItems: 'center', justifyContent: 'center' }]}><AppText style={[styles.itemTitle, { color: theme.text }]}>{t(model.language, 'app.loading')}</AppText></SafeAreaView>;
    ...
    screen = (
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.page }]}>
        ...
        {['home', 'categories', 'insights', 'settings'].includes(model.screen) ? <BottomNav screen={model.screen} theme={theme} language={model.language} onGo={go} /> : null}
          ...
          <View style={styles.celebrateScrim}>
            <View style={[styles.celebrateCard, { backgroundColor: theme.surface, borderColor: theme.line, borderWidth: 1, alignItems: 'stretch' }]}>
              ...
              <AnimatedCue trigger={pendingRestore?.preview.backupDate ?? 'restore-preview'} distance={6}>
                <View style={[styles.card, { backgroundColor: theme.surface2, borderColor: theme.line, marginTop: 18, shadowOpacity: 0, elevation: 0 }]}>
                  {[
                    ...
                    ['Transactions', pendingRestore?.preview.counts.expenses ?? 0],
                    ['Goals', pendingRestore?.preview.counts.goals ?? 0],
                    ['Borrowed-money items', pendingRestore?.preview.counts.ious ?? 0],
                    ['Recurring payments', pendingRestore?.preview.counts.recurringPayments ?? 0],
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginTop: 22 }}>
                <AnimatedPressable accessibilityRole="button" accessibilityLabel="Cancel restore" style={[styles.button, { flex: 1, backgroundColor: theme.surface2 }]} contentStyle={styles.buttonContent} onPress={() => setPendingRestore(null)}><AppText style={[styles.buttonText, { color: theme.text }]}>Cancel</AppText></AnimatedPressable>
                <AnimatedPressable accessibilityRole="button" accessibilityLabel="Restore backup" style={[styles.button, { flex: 1, backgroundColor: theme.primary }]} contentStyle={styles.buttonContent} onPress={restorePendingBackup}><AppText style={styles.buttonText}>Restore</AppText></AnimatedPressable>
              </View>
            </View>
          </View>
        </Modal>
        <Modal visible={celebrate !== null} transparent animationType="fade" onRequestClose={() => setCelebrate(null)}>
          <View style={styles.celebrateScrim}>
            <View style={[styles.celebrateCard, { backgroundColor: theme.surface, borderColor: theme.line, borderWidth: 1 }]}>
              ...
              <TouchableOpacity accessibilityRole="button" accessibilityLabel="Dismiss celebration" style={[styles.button, { backgroundColor: theme.primary, alignSelf: 'stretch', marginTop: 22 }]} onPress={() => setCelebrate(null)}><AppText style={styles.buttonText}>Nice!</AppText></TouchableOpacity>
            </View>
          </View>
        </Modal>
        {renderSheet()}
